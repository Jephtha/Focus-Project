...
public class GameController extends JFrame
{
  ...
  private JLabel topLabel, stackDisplayLabel, stackDisplayText;
  ...
  public GameController(int players, String difficulty, String color)
  {
    ...
    stackDisplayText = new JLabel("");
    stackDisplay.add(stackDisplayLabel);
    stackDisplay.add(stackDisplayText);
    ...
    for (int y=0; y<10; y++)
    {
      for (int x=0; x<10; x++)
      {
        ...
        if ((y==5 & (x==1|x==3|x==5|x==6|x==7|x==8)) | (y==6 & (x==1|x==3)) | (y==7 & (x==3|x==5|x==6|x==7)) | (y==8 & x==3)) {
          Piece piece = new Piece(player2,playerColor[1],x,y,playerColorAsString[1]);
          gameTile[x][y].addPiece(piece);
        }
        else if ((x==5 & (y==1|y==2|y==3|y==4|y==6|y==8)) | (x==6 & (y==6|y==8)) | (x==7 & (y==2|y==3|y==4|y==6)) | (x==8 & y==6)) {
          Piece piece = new Piece(player1,playerColor[0],x,y,playerColorAsString[0]);
          gameTile[x][y].addPiece(piece);
        }
        else if ((x==1 & y==3) | (x==2 & (y==3|y==5|y==6|y==7)) | (x==3 & (y==1|y==3)) | (x==4 & (y==1|y==3|y==5|y==6|y==7|y==8))) {
          Piece piece = new Piece(player3,playerColor[2],x,y,playerColorAsString[2]);
          gameTile[x][y].addPiece(piece);
        }
        else if ((y==1 & x==6) | (y==2 & (x==2|x==3|x==4|x==6)) | (y==3 & (x==6|x==8)) | (y==4 & (x==1|x==2|x==3|x==4|x==6|x==8))) {
          Piece piece = new Piece(player4,playerColor[3],x,y,playerColorAsString[3]);
          gameTile[x][y].addPiece(piece);
        }
      }
    }
  ...
  public void takeTurn()
  {
    if(activePlayers.size()>1)
    {
      ...
      topLabel.setText(activePlayer.getPlayerName()+"'s turn! Click the piece you'd like to move.");
      if(activePlayer.getColor()==Color.green)
      {
        topLabel.setForeground(new Color(0,153,51)); //color darker green;
      }
      else if(activePlayer.getColor()==Color.yellow)
      {
        topLabel.setForeground(new Color(179,170,0)); //color darker yellow;
      }
      else
      {
  ...
  public void selectPiece(GameTile clicked)
  {
    ...
    if(clicked.hasPiece())
    {
      if(clicked.getTopPiecePlayer() == activePlayer)
      {
        ...
        topLabel.setText("You've selected a piece of height: "+steps+". Choose where to move this piece!");
        selected=true;
        prevX = currX;
        prevY = currY;
      }
    ...
    else
    {
      ...
      for (int y=0; y<10; y++)
      {
        for (int x=0; x<10; x++)
        {
          ...
          {
            gameTile[x][y].setBackground(Color.lightGray);
          }
        }
      }
  ...
  public void moveStack()
  {
    ...
    stackDisplayText.setText("/ ");
    stackDisplayText.setAlignmentX(Component.CENTER_ALIGNMENT);
    stackDisplayLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
    stackDisplayLabel.setText("You're moving a stack! Choose how many pieces to move. The pieces in order (top to bottom) are:");
    for(int x=stackHeight-1; x>=0; x--)
    {
      String playerName = gameTile[prevX][prevY].getPieceAt(x).getPlayerName();
      String playerColor = gameTile[prevX][prevY].getPieceAt(x).getColorAsString();
      stackDisplayText.setText(stackDisplayText.getText()+playerName+"'s "+playerColor+" piece / ");
    }
    String[] stackDisplayOptions = {"1","2","3"};
    String stackBottomPiece = (String)JOptionPane.showInputDialog(null,stackDisplay,"Separate Stack",JOptionPane.PLAIN_MESSAGE,null,stackDisplayOptions,"1");
  }
}
